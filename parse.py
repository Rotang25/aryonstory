...
def parse_with_format(parrent, indent=2*'\t'):
    res = []
    for c in parrent.contents:
        if isinstance(c, str):
            res.append(indent + c)
        else:
            res.append(f'{indent}<{c.name}>')
            res.extend(parse_with_format(c, indent + '\t'))
            if not c.isSelfClosing:
                res.append(f'{indent}</{c.name}>')
    return res
...
def parse_post(post):
    ...
    story['text'] = parse_with_format(text)
        ...
        if os.path.isfile(filepath):
            if os.path.splitext(file)[1] == '.html':
                with open(filepath, 'rb') as f:
                    print(filepath)
                    storybook.extend(parse_post(k) for k in BS(f, 'lxml').find('div', class_='blog-posts hfeed').find_all('div', class_='date-outer'))
        else:
